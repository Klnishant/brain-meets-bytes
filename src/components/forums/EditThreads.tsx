...
const EditThread: React.FC<EditThreadProps> = ({threadId,threadTitle,threadContent,threadImages,threadCategories}) => {
  const [title, setTitle] = useState(threadTitle);
  const [content, setContent] = useState(threadContent);
  const [selectedCategories, setSelectedCategories] = useState<Category[]>(threadCategories);
...
const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="rounded-2xl border bg-white p-5 shadow-sm">
      ...
      {/* Category Selector */}
      <SelectCategoryCard onChange={handleCategoryChange} threadCategories={threadCategories} />
      ...
      <div className="mb-4 mt-2 flex items-center gap-4">
        ...
        <span className="text-xs text-[#64748B]">{images.length} selected</span>
      </div>
